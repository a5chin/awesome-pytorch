...
from .containers import Classifier, ClfDataset, Metrics
...
class TorchNet:
    ...
    def train(
        ...
        for epoch in range(total_epoch):
            ...
            with tqdm(
                enumerate(self.train_dataloader, 0), total=len(self.train_dataloader)
            ) as pbar:
            ...
            self.evaluate(model, self.val_dataloader, epoch)

            self.scheduler.step()

        model.load_state_dict(torch.load(self.ckpt / "best_ckpt.pth"), strict=False)
        return model
    ...
    def set_data(
        ...
        self.val_dataloader = DataLoader(
            dataset=val_dataset, batch_size=len_val, shuffle=False, drop_last=True
        )
    ...
    def create_model(
        ...
        Args:
            ...
            act_fin (str, optional): Output layer functions. Defaults to 'Softmax'.
        ...
        """
        model = Classifier(
            layers=layers, act_fn=act_fn, dropout=dropout, act_fin=act_fin
        )
        if init_weights:
